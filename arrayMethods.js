...
const filteredArr = mixedNumbers.filter(function(elem) {
  return elem % 2 === 0
})
...
const insanelyHighTaxes = prices.map((element) => {
  return element * 100000000000000

})
console.log(insanelyHighTaxes)
...
const runningTotal = populations.reduce(function(acc, curr){
  return acc + curr
})
...
const monsterList = monstersInYourPocket.filter(function(elem) {
              return elem.CP > 200
})
